    ...
    <div>
      ...
      <div class="content">
        <div class="left">
          ...
          <div class="icon">
            <span style="cursor: pointer;">
              <i @click="downFile" class="el-icon-download"></i>
            </span>
          </div>
          <div class="icon">
            <span style="cursor: pointer;">
...
export default {
  ...
  methods: {
    ...
    downFile(){
      ...
      if(!id){
        this.$message.error('下载歌曲信息不存在')
        return false
        }  //id不存在 直接返回 
      let songName = this.getNowMusicMenu.name +'.mp3'
      let musicUrl = this.getNowMusic
      let link = document.createElement('a')
      link.style.display = 'none'
      link.href = musicUrl
      link.setAttribute('download',songName)
